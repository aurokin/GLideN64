	...
	const rvk2::RenderWorkPacket & _work)
{
	...
	std::fprintf(
		file,
		"frame=%llu\twork_ordinal=%llu\tsource_packet_id=%llu\top_kind=%u\top_name=%s\tphase=%u\tcolor_image=0x%08X\tx=%u\ty=%u\tprev=0x%08X\tnew=0x%08X\ttexel=0x%08X\tcombiner=0x%08X\tblender=0x%08X\tfinal=0x%08X\ttexture_source_bits=0x%08X\toverwrite_to_black=%u\tquantized_to_black=%u\tpreserved=%u\tprev_write_mask=%u\tfocus_cluster=%u\tfocus_texel_repeat=%u\tfocus_tlut_repeat=%u\tcombine_mux=0x%016llX\tother_modes=0x%016llX\tblend_params=0x%08X\tcycle_type=%u\talpha_compare=%u\tcvg_dest=%u\tblend_mask=0x%02X\tcvg_x_alpha=%u\talpha_cvg_sel=%u\tcolor_on_cvg=%u\tforce_blender=%u\tdepth_source=%u\tdepth_test=%u\tdepth_compare_enable=%u\tdepth_update_enable=%u\tscissor_mode=%u\ttexrect_flip=%u\tprim_color=0x%08X\tenv_color=0x%08X\tblend_color=0x%08X\tfog_color=0x%08X\tfill_color=0x%08X\ttile=%u\ttile_format=%u\ttile_size=%u\ttile_line=%u\ttile_tmem=%u\ttile_palette=%u\ttile_cmt=%u\ttile_cms=%u\ttile_maskt=%u\ttile_masks=%u\ttile_shiftt=%u\ttile_shifts=%u\ttexture_image_format=%u\ttexture_image_size=%u\ttexture_image_width=%u\ttexture_image_address=0x%08X",
		...
		_work.blendParams,
		static_cast<unsigned>(_work.cycleType),
		static_cast<unsigned>(_work.alphaCompare),
		static_cast<unsigned>(_work.cvgDest),
		static_cast<unsigned>(_work.blendMask),
		static_cast<unsigned>(_work.cvgXAlpha ? 1U : 0U),
		static_cast<unsigned>(_work.alphaCvgSel ? 1U : 0U),
		static_cast<unsigned>(_work.colorOnCvg ? 1U : 0U),
		static_cast<unsigned>(_work.forceBlender ? 1U : 0U),
		static_cast<unsigned>(_work.depthSource),
		static_cast<unsigned>(_work.depthTest ? 1U : 0U),
		static_cast<unsigned>(_work.depthCompareEnable ? 1U : 0U),
		static_cast<unsigned>(_work.depthUpdateEnable ? 1U : 0U),
		static_cast<unsigned>(_work.scissorMode),
		static_cast<unsigned>(_work.texRectFlip ? 1U : 0U),
		_work.primColor,
		_work.envColor,
		_work.blendColor,
		_work.fogColor,
		...
		static_cast<unsigned>(_work.tilePalette),
		static_cast<unsigned>(_work.tileCmt),
		static_cast<unsigned>(_work.tileCms),
		static_cast<unsigned>(_work.tileMaskt),
		static_cast<unsigned>(_work.tileMasks),
		static_cast<unsigned>(_work.tileShiftt),
		static_cast<unsigned>(_work.tileShifts),
		static_cast<unsigned>(_work.textureImageFormat),
		static_cast<unsigned>(_work.textureImageSize),
	...
	const std::vector<u32> * _workTMEMSnapshotIndices)
{
	...
	if (it != surfaces.end()) {
		...
		const bool autoHistoryPresentLiveMerge =
			viMatchedHistorySelection && frameHasLiveSurfaceWrites;
		const bool historyPresentMergeEnabled =
			autoHistoryPresentLiveMerge
			|| debugEnableUntouchedPresentCarry()
		...
		if (historyPresentMergeEnabled
			...
			const auto canMergeLiveCandidate = [&](u32 _candidateAddress, const ColorSurface & _candidate) {
				if (_candidateAddress == cached.address
					|| (isDepthOnlyColorAddress(_candidateAddress)
						&& !allowDepthAliasedHistoryCarry)
					|| _candidate.format != cached.format
					|| _candidate.size != cached.size
					|| _candidate.width != cached.width
				...
				const auto writesIt = surfaceColorWrites.find(_candidateAddress);
				return writesIt != surfaceColorWrites.end() && writesIt->second > 0ULL;
			};
			struct LiveMergeCandidate
			{
				u32 address = 0U;
				u64 writes = 0ULL;
				u64 works = 0ULL;
			};
			std::vector<LiveMergeCandidate> liveMergeCandidates;
			if (autoHistoryPresentLiveMerge) {
				liveMergeCandidates.reserve(surfaces.size());
				for (const auto & liveEntry : surfaces) {
					const u32 liveAddress = liveEntry.first;
					const ColorSurface & liveSurface = liveEntry.second;
					if (!canMergeLiveCandidate(liveAddress, liveSurface))
						continue;
					const auto writesIt = surfaceColorWrites.find(liveAddress);
					const auto worksIt = surfaceWorkCounts.find(liveAddress);
					liveMergeCandidates.push_back(LiveMergeCandidate{
						liveAddress,
						writesIt != surfaceColorWrites.end() ? writesIt->second : 0ULL,
						worksIt != surfaceWorkCounts.end() ? worksIt->second : 0ULL
					});
				}
				std::sort(
					liveMergeCandidates.begin(),
					liveMergeCandidates.end(),
					[](const LiveMergeCandidate & _a, const LiveMergeCandidate & _b) {
						if (_a.writes != _b.writes)
							return _a.writes > _b.writes;
						if (_a.works != _b.works)
							return _a.works > _b.works;
			...
			for (const auto & historyEntry : m_surfaceHistory) {
				...
				for (const ExecutorCachedSurface * existing : historyMergeCandidates) {
					if (existing != nullptr && existing->address == candidate.address) {
			...
			summary.selectedPresentSurfaceHistoryMergeCandidateCount =
				static_cast<u64>(historyMergeCandidates.size() + liveMergeCandidates.size());
			...
			const bool copyNonBlackFromLiveCandidates = debugHistoryMergeCopyNonBlack();
			const auto mergeHistoryCandidatePixels =
				...
				for (size_t i = 0U; i < presentPixelCount; ++i) {
					...
					if (!destinationVisible)
						...
						++potentialNonBlackDiff;
					if (destinationVisible && !_copyNonBlack)
						continue;
			...
			for (const LiveMergeCandidate & candidate : liveMergeCandidates) {
				const auto liveIt = surfaces.find(candidate.address);
				if (liveIt == surfaces.end())
					continue;
				const ColorSurface & live = liveIt->second;
				mergeHistoryCandidatePixels(
					candidate.address,
					live.pixels,
					&live.coverage,
					&live.hiddenCoverage,
					copyNonBlackFromLiveCandidates);
			}
