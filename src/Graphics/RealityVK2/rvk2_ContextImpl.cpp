...
void appendFrameForensicsRecord(const rvk2::ExecutorOutput & _output)
{
	...
	for (u32 i = 0U; i < rvk2::kExecutorStageDeltaClassBuckets; ++i) {
		std::fprintf(
			file,
			"\tstage_cls%u_writes=%llu\tstage_cls%u_t2f=%llu\tstage_cls%u_c2b=%llu\tstage_cls%u_txrepl=%llu\tstage_cls%u_txtmem=%llu\tstage_cls%u_txrdram=%llu\tstage_cls%u_txsynth=%llu",
			i,
