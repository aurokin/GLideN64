...
:: tracking
set TPSM=RealityVK\projects\msvc\
set EXIT=exit
...
type nul
set QTURL=https://github.com/gonetz/RealityVK/releases/download/qt_build
...
for /f "tokens=1" %%R in ('git rev-parse --short HEAD') do set "REV=RealityVK-%%R-"
set "IDTS=-%ARCH%"
if "%CONF%" == "Debug" set "IDTS=%IDTS%-dbg"
if defined TOOLSET set "IDTS="-%TOOLSET%%IDTS%""
set IDTS=%IDTS:"=%
set "PJ64QT=%BUILDROUTE%\%REV%%PJREN%-Qt%IDTS%"
::set "M64QT=%BUILDROUTE%\%REV%Mupen64Plus-Qt%IDTS%"
set "PJ64WTL=%BUILDROUTE%\%REV%%PJREN%-WTL%IDTS%"
...
:CINI
	...
	type nul
	copy /y ini\RealityVK.custom.ini "%~1\"
	%ERR%
...
:HELP
echo.
echo RealityVK's simplified MSVC build and packing script
echo.
echo Usage:
echo   set ^<variable^>
...
echo   --nopjqt    To skip Project64/PJ64Legacy Qt builds
::echo   --nomqt     To skip Mupen64Plus Qt builds
echo   --nopjwtl   To skip Project64/PJ64Legacy WTL builds
